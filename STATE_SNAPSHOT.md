...
### 42.4 Round 2 (same session): US/Canada medicine test guidance + Biomedicine field

**US/Canada medicine "separate standard test" (founder request, researched):** US & Canadian medicine is graduate-entry MD — MCAT (472–528) is the standardized test; CASPer (situational-judgement, no numeric score) is additionally required by 12 of 17 Canadian MD programs and ~50 US MD/DO programs (~⅓). SAT/ACT applies only to the pre-med bachelor's / rare direct BS-MD routes. Sources: AFMC admission-requirements 2026, AAMC, BeMo/StudyCasper/MYLS school lists. Changes: [StepTests.tsx](../src/components/form/StepTests.tsx) med-test hint rewritten to state the graduate-entry pathway + CASPer explicitly (no CASPer score field — quartile results go directly to schools; capturing a number would invent comparability); [profile-score.ts](../src/lib/profile-score.ts) SAT lever detail is now medicine-aware for USA/Canada aspirants (SAT = pre-med stage, MCAT later). MCAT option/range already existed; no scoring change (med tests stay collected-not-scored).

**Biomedicine = 31st field (founder request):** decisions — Biomedical ENGINEERING stays in Biotech (founder); veterinary-context stays Ag&Vet; informatics/computational stays AI/CS; UG Biomedicine UNGATED (founder; 3-of-4 stays Medicine-only), PG gated on the Biotech life/health-sciences pool. [add-biomedicine-field.py](../scripts/data-fixes/add-biomedicine-field.py) moved **86 programs** (65 Biotech / 16 Natural Sciences / 5 Medicine; 25 engineering-named skipped; 6 spec-only track mentions stay under their broad parent per most-specific-primary policy). Touched: types.ts FIELDS_OF_STUDY (31), scoring.ts FIELD_NAME_PATTERNS + dormant RELATED_FIELDS, field-prereq.ts (PG regex + STEM_PG_FIELDS), roi-data.ts (union + 11 country salary tables mirroring Biotech's sourced figures — biomed-specific sourcing pass pending), options-lenses.ts selectivity 1.05, db-stats regenerated (totalFields 30 → 31; homepage proof strip picks it up). Matcher smoke: PG Biomedicine 13 matches {8 safe/3 reach/2 ambitious}, UG PCB 16, B.Com PG 0 (gate works). +3 vitest (103 total). 375px Playwright: Biomedicine in dropdown, UG PCB ungated, med-test copy renders, no overflow.

### 42.5 Round 3 (same session): Interview Coach overhaul + Australia visa fee

Founder requests (14 Jul 2026), all shipped:

1. **Feedback ≤5s**: route trimmed to max_tokens 500, bullets capped at 15 words, sample answer ≤120 words, coverage reported as index lists. Measured with the exact prompt shape: 3.3-4.2s (was ~4-5.2s; 900-token budget before). Haiku 4.5 unchanged.
2. **Key-point readiness (75% rule)**: every question already carries a priority-ordered official checklist ([src/data/interview-guidelines.ts](../src/data/interview-guidelines.ts) — UK per-question, USA per-section, AU per-category). The model now returns a first-line `COVERAGE: covered=[..] missing=[..]` by index; [src/lib/interview-readiness.ts](../src/lib/interview-readiness.ts) computes the verdict DETERMINISTICALLY (≥75% → ready; unclassified points count missing; malformed line → no verdict, plain feedback). UI: emerald/amber readiness banner with the full priority-ordered ✓/✗ key-point list; spoken feedback opens with the verdict + names the missing points. 7 new vitest specs (110 total).
3. **"Hash hash"/"Asterisks" spoken aloud**: three nets — system prompt forbids markdown, stripMarkdown() server-side, client clean() + speechFriendlyText() strip #*_`~ before TTS.
4. **Feedback starts 5s after the student stops**: silence auto-submit 8000→5000ms (history: 3s→8s→5s founder rule). Interim results still reset the countdown.
5. **TTS spelling out TOEFL/IELTS**: ACRONYM_PRONUNCIATIONS respell map (TTS-only, display untouched): TOEFL→toefel, IELTS→eye-elts, UCAT/GAMSAT/NEET/SEVIS/OSHC/CoE/I-20; "TOEFL/IELTS" slash spoken as "or". Applied inside utterAndAdvance so every speak path is covered.
