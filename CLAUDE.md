...
- **Medicine split from Public Health** (30 fields): 114 clinical / 395 PH; legacy combined value auto-expands in the matcher; **founder-locked 14 Jul 2026: the two fields are permanently separate — never re-merge**. UG Medicine needs any 3 of Physics/Chemistry/Biology/Maths (Maths NOT mandatory; **the 3-of-4 rule is Medicine-ONLY — founder decision 14 Jul 2026, do not extend to Biotech / Agriculture & Veterinary**); **Public Health PG-gated since 14 Jul 2026** (same health-sciences prereq regex as Medicine; UG Public Health has no subject rule); med-test capture (UCAT/MCAT/GAMSAT/HPAT/NEET) on StepTests — **UCAT is the new format: AR subtest removed, total 900–2700 (14 Jul 2026)**; SAT improvement lever suppressed for UG Medicine unless prefs include USA/Canada/Singapore.
...
4. `field_of_study` must be one of the 31 in `FIELDS_OF_STUDY`.
...
| `src/app/page.tsx` | **The homepage** (v3 redesign swap, 14 July 2026): 6-section SERVER component in emotional-journey order — hero (readiness-preview card: score ring + 3 pillars, tier chips REMOVED in the 14 Jul 2026 numerical-density pass), proof strip (+ founder row), how-it-works ("From uncertainty to a decision you can trust"), journey-tools USP section, Parent Decision Room (INR at ₹94/USD), closing CTA; grouped footer. Plain-language pass (founder, 14 Jul 2026): "Admission fit" not "Admissibility", "time to recover your investment" not "earning-back timeline", "scholarship potential" not "scholarship signals", "verified best-fit matches" not "evidence-backed matches" — ON MARKETING SURFACES ONLY (reports/tools keep technical terms). Source-policy badge is "Official sources · third-party benchmarks labelled" (never the absolute "Official sources only"). Safe/Reach/Ambitious carries the decision-support-not-guarantee clarifier. Founder descriptor: "Founder, eduvianAI · Former Regional Director and Global Leadership Team member at IDP Education" (founder added GLT membership 14 Jul 2026 — supersedes the earlier "NOT global leadership" phrasing lock for the DESCRIPTOR only; the letter itself is unchanged). Client islands: ChatWidget, LogoutButton, MobileNav. Pre-v3 page at `_archive/page-pre-v3-swap.tsx.bak` (gitignored, also in git history). Founder letter at `src/app/why-eduvianai/page.tsx`. Voice rules: never "AI-powered" in copy (brand name + AISA carry the signal); CTA is "Check my readiness" (never "See if I qualify"). |
| `src/lib/types.ts` | Single source of truth. `TARGET_COUNTRIES` (12, incl. Netherlands since 10 July 2026), `FIELDS_OF_STUDY` (31, incl. Biomedicine since 14 Jul 2026). |
...
**Founder decisions RESOLVED (14 Jul 2026 — all shipped except hardening):**

3. ~~3-of-4 extension~~ — NO: the rule is Medicine-only. Biotech / Agriculture & Veterinary stay Math-mandatory PCM. · ~~logo.svg~~ — stays violet as-is. · ~~nav "How it works"~~ — reinstated (desktop nav + MobileNav, → /methodology). · ~~Public Health gating~~ — GATED at PG (same health-sciences prereq as Medicine; UG ungated).
4. **Registration-gate hardening: decided but ON HOLD.** Founder chose the any-signed-in-account model (full shared view needs viewer signed in to ANY account AND submitter registered; PII still masked for non-owners; PDF/email owner-only) but deferred the required `user_sessions.authenticated` migration — do NOT implement until founder greenlights. Guest-submit owner cookies must never count as authenticated when it ships; code must fail toward current behavior while the column is absent.
5. UCAT new format shipped (900–2700, AR removed) — StepTests + Zod ceiling.
