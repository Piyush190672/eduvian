  ...
  return (
    <div className={`mt-4 rounded-2xl border border-emerald-200 bg-emerald-50/60 px-4 py-3 ${className}`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-emerald-800">Source proof</p>
      </div>
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-xs text-emerald-900/90">
        {lines.map((l) => (
      ...
      <p className="mt-2 text-xs text-emerald-800/90 leading-snug">{note}</p>
      {sourceUrl && (
        <a
          href={sourceUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-emerald-700 hover:text-emerald-900 transition-colors"
        >
