// Country list for the profile flow.
//
// India is pinned first (almost all users are Indian). The remainder is
// alphabetised by `name`. Each entry carries an E.164 dial code so the
// phone input can auto-align its country-code prefix when the user picks
// a citizenship.
//
// Dial codes are the country trunk (calling) code without leading +.
// Multi-zone countries (USA/Canada share +1, several +44 dependencies)
// keep the conventional code so the dropdown stays predictable.
...
const REST: Country[] = [
  ...
  { name: "Türkiye", code: "TR", dialCode: "90" },
...
export const INDIA: Country = { name: "India", code: "IN", dialCode: "91" };

// India pinned first, rest alphabetical.
export const COUNTRIES: Country[] = [INDIA, ...REST];

// Lookup by exact country name. Returns India for legacy values like
// "Indian" (the demonym) so we degrade gracefully on old drafts.
export function findCountryByName(name: string | null | undefined): Country | undefined {
  ...
  if (/^indian?$/i.test(name.trim())) return INDIA;
  return undefined;
}

// Split a free-text phone like "+91 9876543210" or "919876543210" into
// {dialCode, number}. Falls back to the supplied default dial code (and
// the whole input as `number`) when no recognisable prefix is found.
export function splitPhone(raw: string | null | undefined, fallbackDial: string = INDIA.dialCode): { dialCode: string; number: string } {
  const input = (raw ?? "").trim();
  if (!input) return { dialCode: fallbackDial, number: "" };
  const cleaned = input.replace(/[\s\-().]/g, "");
  const withPlus = cleaned.startsWith("+") ? cleaned : "";
  const digits = cleaned.replace(/^\+/, "");
  // Try longest-prefix match against known dial codes (4 → 1 digits).
  const dialCodes = new Set(COUNTRIES.map((c) => c.dialCode));
  for (let len = 4; len >= 1; len--) {
    const candidate = digits.slice(0, len);
    if (dialCodes.has(candidate) && (withPlus || /^\+/.test(input))) {
      return { dialCode: candidate, number: digits.slice(len) };
    }
  }
  // No +prefix at all → treat whole string as the local number.
  return { dialCode: fallbackDial, number: digits };
}
